...
## Урок 4: запись и транзакции

45. [0045 — очередь эффектов проверена через БД](0045-uow-behavior-tests.md): пользовательские тесты проверяют отсутствие
    записи до commit, запись после commit и атомарный откат при ошибке второй вставки. Различие транзакции и UoW
    пока остаётся предметом объяснения, а не подтверждённым пониманием.

Связанный материал: [урок 0014](../lessons/0014-transactions-and-unit-of-work.html) и
[памятка](../references/transactions-and-uow.html).

46. [0046 — конфликт версии не перезаписывает строку](0046-optimistic-update-conflict.md): пользовательский тест
    проверяет увеличение версии, отдельную ошибку и сохранность данных на SQLite; реализация выполнена с помощью.
47. [0047 — порядок вставки по FK в commit](0047-e4-3-insert-ordering.md): алгоритм Кана, рёбра из relations,
    `register(table, effect)` и сортировка очереди; тест на SQLite с FK и две мутационные проверки; с помощью.
