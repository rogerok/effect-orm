...
**L3.22: протянуть nullability в возвращаемый тип `col`.**

L3.21 закрыт: `SourceMap` хранит `Source<T, N>` вместо `TableDef`, `selectFrom` и `innerJoin` ставят `false`,
`leftJoin` — `true`, флаги подтверждены утверждениями в `src/hw/e3-2.test.ts` и мутационной проверкой. Подробности,
четыре промежуточные ошибки и семантика `ON` — в [записи 0040](records/0040-l3-21-source-nullability-metadata.md).

Условный тип в `col` уже написан, но не проверен: утверждений на возвращаемый тип `col` нет, реальная несовпавшая
строка после правки типов не наблюдалась.
...
| LEFT JOIN nullability                          |       2 | Метаданные источника реализованы, но форма `SourceMap` исправлялась четыре раза; семантика `ON` выбрана верно и названа по ощущениям, правило дал наставник. Запись 0040. |
...
Признак nullability источника проверен на уровне метаданных (запись 0040); влияние признака на тип выражения ещё нет.

## Ближайшая учебная цель

L3.22 — сделать nullability видимой в типе выражения:

```text
inner source → col('u', 'name')  : Expr<string>
left source  → col('c', 'body')  : Expr<string | null>
```

Условный тип в `col` уже написан. Не хватает двух наблюдений: утверждений `expectTypeOf` на возвращаемый тип `col` для
inner и left источников и реальной несовпавшей строки, где значение и тип совпадают.
